...
app is deployed. Leave `SINK_DB_*` unset so Sink uses the exact default connection and transaction manager.
...
| `SINK_DB_HOST/PORT/DATABASE/USERNAME/PASSWORD` | unset by default | Set all five only for an intentional split metadata database. Setting them, even to the same values as `DB_*`, creates a distinct connection/PDO. |
