...
The root app is the deployable Laravel shell and requires the local split packages through Composer
path repositories. Package conventions mirror Hone/Matte: each package owns `composer.json`, `src/`,
`tests/`, `phpunit.xml`, `README.md`, and `LICENSE`, with `sink-client` and `sink-server` exposing
auto-discovered Laravel service providers.

Package namespaces:

- `ArtisanBuild\SinkContracts\` => `packages/sink-contracts/src/`
- `ArtisanBuild\SinkClient\` => `packages/sink-client/src/`
- `ArtisanBuild\SinkServer\` => `packages/sink-server/src/`

## Gate

Before reporting a feature branch green, run `composer ready` and `composer packages:check` from the
root. The gate must be idempotent: commit any rector, Pint, ide-helper, or lockfile changes it
produces so a second run leaves no diff.
